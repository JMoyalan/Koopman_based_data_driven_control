...
Div_F.coefficient(find(abs(Div_F.coefficient) <= 1e-6)) = 0;
Div_G.coefficient(find(abs(Div_G.coefficient) <= 1e-6)) = 0;
% Div_F = sum(diag(jacobian(fx,x)));
% Div_G = 0;

term(1) = C_a_Psi'*L1*Psi + C_a_Psi'*Psi*Div_F;
term(2) = C_c_Psi'*(L2-L1)*Psi + C_c_Psi'*Psi*Div_G;
term(3) = C_ab_Psi'*L1*Psi + C_ab_Psi'*Psi*Div_F;
term(4) = C_bc_Psi'*(L2-L1)*Psi + C_bc_Psi'*Psi*Div_G;
...
pconstr(1) = sos_poly >= gamma*ax*bx;; % Dual Lyapunov stability
pconstr(2) = sx - cx >= 0;
pconstr(3) = sx + cx >= 0;
if opts.OCP.deg_a(end) ~= 0
    pconstr(4) = ax >= 0; % rho(x) >= 0
end
...
% Draw a vector field
% -----------------------------------------------------------------------
[y1i, y2i] = meshgrid(y1s:dy:y1e, ys:dy:ye);
yi = [y1i(:), y2i(:)];
